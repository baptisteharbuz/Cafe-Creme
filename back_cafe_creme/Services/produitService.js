...
const addProduit = (produit) => {
    return new Promise((resolve, reject) => {
        const sql = "INSERT INTO Produit(PR_Label, PR_Prix, FK_Type_Produit) VALUES (?, ?, ?);"
        conn.query(sql, [produit.Label, produit.Prix, produit.Type], (err, result, field) => {
            if (err) return reject(err)
            resolve(result)
        })
...
const deleteProduit = (id) => {
    return new Promise((resolve, reject) => {
        const sql = "DELETE FROM Produit WHERE PR_ID = ?;";
